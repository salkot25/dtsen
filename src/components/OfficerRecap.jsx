...
export default function OfficerRecap({ officers = [] }) {
  ...
  // 1. KPI Aggregations
  const stats = useMemo(() => {
    if (!officers || officers.length === 0) return { total: 0, submitted: 0, open: 0, avgRealisasi: 0 };
  ...
  const processedOfficers = useMemo(() => {
    ...
    return list;
  }, [officers, searchTerm, performanceFilter, sortBy, sortOrder]);
  ...
  if (!officers || officers.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center">
        ...
        <h3 className="text-lg font-bold text-slate-800">Data Rekap Petugas Kosong</h3>
        <p className="text-sm text-slate-500 mt-1 max-w-md mx-auto">
          Belum ada data rekap yang dimuat. Silakan unggah berkas rekap Excel (`.xlsx`) di menu <strong>Input Laporan</strong> untuk memulai.
        </p>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="block w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors shadow-inner"
            />
          </div>
          ...
          <div className="flex flex-wrap w-full md:w-auto items-center gap-3">
            ...
            <select
              ...
              className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-slate-700 font-medium"
            >
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-xl p-1">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition-colors ${
                  sortBy === 'name' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                Nama Biller
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition-colors ${
                  sortBy === 'submitted' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                Submitted
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition-colors ${
                  sortBy === 'realisasi' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                Realisasi
                      ...
                      <td className="py-3.5 px-6 text-center font-bold text-slate-500">
                        {rank <= 3 && sortOrder === 'desc' && sortBy === 'realisasi' ? (
                          <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs text-white font-extrabold ${
                            rank === 1 ? 'bg-amber-400 shadow-md shadow-amber-400/20' : 
